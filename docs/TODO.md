...
- [ ] **Endorse→Attestation cutover (Slice E obligation, opened 2026-06-25)** — one business concept ("I back this subject") is implemented twice and both are live: legacy `EndorsementService` writes `wp_bcc_trust_endorsements` and *moves the score* (`endorsement_bonus`); new `AttestationService` writes `wp_bcc_trust_attestations` (kind `vouch`/`stand_behind`) with **zero score impact** until Slice E. Correct as a migration state, a liability if it calcifies. **When Slice E lands** (the People/self-page redesign wires attestations into the score), in the SAME commit: (a) re-source `endorsement_bonus` from `wp_bcc_trust_attestations` via `TrustScoreService`, (b) delete the `EndorsementService` write paths, (c) translate the member identity with `MemberSelfPageService::selfPageId(user_id)` at the attestation→score writer — attestations key the member as `target_kind='user_profile', target_id=raw user_id` (no `ID_BASE` offset) while the score row is keyed `page_id = ID_BASE + user_id`; the Phase-2 bridge assert (`testUserProfileAttestationTargetResolvesToSelfPage`) guards this but the *score-write* path must call the translation explicitly. Refs: `app/Domain/Core/Services/EndorsementService.php`, `AttestationService.php`, `MemberSelfPageService.php:41`, `AttestationRepository.php:53-58`. (Domain-audit F1/F2; identity bridge already shipped Phase 2 `c8de098`.)
