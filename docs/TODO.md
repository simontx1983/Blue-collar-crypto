...
- [ ] **Locals endpoints — contract says join/leave/primary, code uses `/membership` + POST.** [`api-contract-v1.md:2153`](api-contract-v1.md) claims `POST /me/locals/:id/join` and `DELETE /me/locals/:id`; [`LocalsEndpoint.php:128`](../app/public/wp-content/plugins/bcc-trust/app/Domain/Core/REST/LocalsEndpoint.php#L128) actually registers `POST`/`DELETE /me/locals/:id/membership` for join+leave. [`api-contract-v1.md:2185`](api-contract-v1.md) claims `PUT /me/locals/:id/primary`; [`LocalsEndpoint.php:101`](../app/public/wp-content/plugins/bcc-trust/app/Domain/Core/REST/LocalsEndpoint.php#L101) registers `POST` (and there's also a `DELETE /me/locals/primary` clearer that the contract doesn't document). Recommended fix: update contract to match code (the `/membership` verb is a deliberate API design choice).
- [ ] **Admin ranks endpoints — contract claims, code missing.** [`api-contract-v1.md:2500`](api-contract-v1.md) claims `POST /admin/ranks/award` and [`:2527`](api-contract-v1.md) claims `DELETE /admin/ranks/:rank/:user_id`. Grep shows zero `register_rest_route` calls for either path. Either build them or strike from the contract.
