...
| `bcc_rank_stewardship_sweep` (`RankScheduler::EVENT_STEWARDSHIP_SWEEP`) | `bcc_weekly` | [RankScheduler.php](../app/public/wp-content/plugins/bcc-trust/app/Domain/Rank/Services/RankScheduler.php) (same self-heal boot) | `StewardshipSweepService` — Rank helping emitter (2026-08-04): credits owners of genuinely-active User-kind communities (`actives ≥ 5` distinct 7-day posters via `getActivityHeat`) with `stewardship` evidence (→ contribution + helping), idempotent per (group, ISO-week), responsible-owner-gated (not suspended/fraud/recovery). Bounded batch (100 / 30s). Failure records `rank_scoring`/`evidence_ingest_failed`; writes `bcc_rank_stewardship_sweep_last_run`/`_last_success` heartbeat options. |
...
| ~~`bcc_nft_enrichment_tick`~~ | ~~`bcc_five_minutes`~~ | **RETIRED 2026-09-03 (PR 7.1)** | `\BCC\Trust\Onchain\Services\NftEnrichmentService::CRON_HOOK`. **No longer scheduled and no longer bound to a handler.** It ran every five minutes, iterated EVERY active chain via `runAllChains()`, and called that chain's metadata provider for any row with a NULL `collection_name` — with no capability gate, no per-chain opt-in, no administrator-created discovery run, and independent of `BCC_COSMWASM_DISCOVERY_ENABLED`. ⚠ It was dormant only by DATA (zero pending rows measured on staging and production on 2026-09-03), not by design: a discovery scan emits `collection_name => null` for any CW-721 whose `contract_info` carries no name, which would have handed this loop provider work on a chain *cron* picked. All three registration sites are gone (handler binding, `plugins_loaded` self-heal, activation schedule) and `register()` is a documented no-op. Cleared from live installs by the `unschedule_automatic_nft_discovery_v2` migration entry — a SECOND entry with its own `done_option`, because v1's is already set everywhere it ran. The service itself is preserved for an explicit administrator action; `findPendingEnrichment()` is deliberately not broadened. |
| `bcc_watch_batch_sweep` (`WatchBatchAggregator::SWEEP_HOOK`; legacy hook name `bcc_pull_batch_sweep` retired 2026-05-13 — self-heal `wp_clear_scheduled_hook` on `plugins_loaded`) | `bcc_minute` (every minute; `WatchBatchAggregator::SWEEP_INTERVAL`) | [bcc-trust/app/Domain/Core/Plugin.php](../app/public/wp-content/plugins/bcc-trust/app/Domain/Core/Plugin.php) — `Plugin::registerAsyncJobs()` | `Plugin::registerAsyncJobs()` registers the `cron_schedules` filter, schedules `WatchBatchAggregator::SWEEP_HOOK` at `::SWEEP_INTERVAL`, and dispatches to `watchBatchAggregator()->sweep()`. Cited by symbol, not line number: the previous entry pointed at `Plugin.php:1320`/`:1327` and had drifted ~640 lines, and claimed an interval slug (`bcc_pull_batch_sweep_minute`) that never existed anywhere in the codebase. |
