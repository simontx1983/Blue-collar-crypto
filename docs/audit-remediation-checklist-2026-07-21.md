...
### ☐ CL-FN06 — **FN-06** · `public_all` authorization (use vs. manage) — **OPEN** · backend **IN PROGRESS (prepared/uncommitted, locally verified)** · FE **OPEN**
- **Priority:** P2 · **Timing:** before-production · **Type:** code (backend + frontend) + test · **Surface:** bcc-trust + bcc-frontend
- **F-IDs:** **FN-06** (Phillip's 2026-07-22 authorization decision — resolved policy; implementation not yet merged)
- **Sub-status:** backend implementation = **PREPARED / UNCOMMITTED** (locally verified — PHPUnit/PHPStan/guards green; **not** committed, pushed, deployed, or PR'd, so **not** DONE) · FE composer disclosure + owner-toggle UI = **OPEN** · final completion = **NOT REACHED**. Only after the backend PR **merges** may the backend portion be marked DONE with PR # + merge SHA. CL-FN06 as a whole stays OPEN while FE work is in its acceptance criteria.
- **One-post/one-origin** (architecture): **RESOLVED as an arch decision** (zero-or-one scalar group per post; `public_all` syndicates via visibility only, never a second origin). Its enforcement code + any UI remain tracked here, not marked shipped.
- **Location:** `PostsService::gateGroupPost` (now `(int,int,string)`), `GroupsService::canUsePublicAll` / `canManagePublicAllPolicy` / `setPublicAllMembersPolicy`, `PublicAllPolicy` (new, Core/Support), `GroupPostPolicyRepository` (new, Core/Repositories), `MyGroupsEndpoint POST /me/groups/:id/post-policy` (new route).
- **Prior behavior (2026-07-22 trace):** non-members cannot post at all (membership required); **any** active posting member could select `public_all` — no role restriction; no manage capability existed.
- **Approved policy (RESOLVED — Phillip 2026-07-22), two distinct authorizations:**
  - **USE `public_all` on one's own post** (`canUsePublicAll`): open → any posting member; closed/secret → owner/manager/**moderator**, or ordinary member iff opted in; readonly/pending/banned/non-member → deny.
  - **MANAGE the group opt-in** (`canManagePublicAllPolicy`): owner / manager / **site-admin (`manage_options`)** only. **Moderator excluded** (may use, may not manage). ordinary/readonly/non-member/anon → deny.
- **Backend implementation (PREPARED / UNCOMMITTED — 2026-07-22):**
  - **Decision core** `PublicAllPolicy::canSyndicate` — pure, fail-closed (use-set = {owner,manager,moderator}; open member allow; closed/secret member = opt-in).
  - **Enforcement** `$visibility` threaded into the single shared `gateGroupPost`, called by all three writers before content/throttle. Ineligible `public_all` → **REJECTED** `bcc_permission_denied` (not down-clamped); backend-authoritative. Filter `bcc_group_public_all_denied`.
  - **Manage authz** canonical `canManagePublicAllPolicy` (owner/manager/site-admin; moderator excluded) — single source of truth for BOTH the write route AND the `can_manage_public_all_policy` response flag (no role logic re-derived in response shaping). Write route uses `resolveGroupAccess` first → secret group never leaks existence (404, not 403).
  - **Opt-in storage** `GroupPostPolicyRepository` — `_bcc_group_public_all_members` post-meta on the `peepso-group` CPT (GatedGroupRepository pattern; no schema/migration). Default off.
  - **Owner control** route `POST /me/groups/:id/post-policy` (owner/manager/site-admin; strict boolean; contract §4.7.3).
  - **Eligibility read** `can_use_public_all` + `can_manage_public_all_policy` + `public_all_members_enabled` on the group-detail view-model (§4.7). Minimum exposure: the raw opt-in is real only for managers; other viewers see `false`.
  - **One-origin** verified: `group_id` scalar (REST `type: integer` rejects arrays/malformed pre-handler); `PeepSoStatusWriter:173-175` stamps one `peepso_group_id`; `public_all` sets only `_bcc_post_visibility`.
  - **Testability** `GroupsService` takes `GroupContextResolver` via constructor injection (matches `LocalsService`; behavior-preserving).
- **Tests (all green, local):** `PublicAllPolicyTest` (full use-matrix) · `GroupPostPolicyRepositoryTest` (default-off + round-trip) · `GroupsPublicAllGateTest` (use + **manage role matrix, every role explicit** + secret no-leak + lifecycle) · `WriterVisibilityParityTest` (status/photo/gif each: reject-no-writer, eligible-proceeds, members_only/public_group unchanged, Floor bypass) · `RestDestinationValidationTest` (arg-schema one-origin guard) · `MyGroupsPostPolicyRouteTest` (route auth/roles/secret-404/lifecycle/envelope). ~119 public_all tests; full bcc-trust Unit suite green; PHPStan L8 clean; contract-parity + cadence guards pass.
- **OPEN follow-up (FE PR — bcc-frontend):** composer **hide/disable** `public_all` for an ineligible author (`can_use_public_all`); owner **toggle** wired to the route (`can_manage_public_all_policy` + `public_all_members_enabled`); `public_all` disclosure/warning (CL-FN05 territory). No backend blocker — API ready. HTTP/staging probe of the route + fields pending.
- **Deps:** CL-7C (policy) · **Risk:** medium (authorization) · **Size:** medium · **Tracker:** this item · **Evidence:** 2026-07-22 backend implementation report. **Backend prepared/uncommitted — awaits Phillip's review before commit/PR; mark DONE only post-merge.**
