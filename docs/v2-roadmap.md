...
| Cosmos NFT support (CW-721) | Cosmos creator coverage via standard CW-721 LCD queries | Per-chain fetcher | SHIPPED 2026-05-07 — `feat/v2-phase-2-cosmos-cw721`; intentionally asymmetric with ETH/SOL persistence (read-time + V1 transient, see `project_v2_phase_2_cosmos.md`). UPDATED 2026-07-02 — Stargaze L1 halted (Prop-1017 Hub migration); chain row retired, collections re-curated on the Cosmos Hub, top-collections auto-discovery deleted (Constellations API went partner-only). |
...
| **Sponsorship management** (logged 2026-07-21, admin audit) | Operator record-keeping for sponsor deals — contacts, amounts, terms, dates. Internal-only; never rendered to users in this scope. | One wp-admin CRUD page + table + repository. Deliberately NO REST route, NO api-contract change, NO frontend work. | OPEN — deferred post-launch by Phillip 2026-07-21. Placement decided: **wp-admin cockpit** (per the §8 admin split — low-frequency bookkeeping, not a daily workflow; the Next.js `/admin/*` ops center is the wrong surface). Preliminary shape (from `docs/admin-audit-2026-07-21.md`): table `wp_bcc_sponsorships` via `bcc-trust/includes/database/schema-sponsorships.php` (sponsor name, contact name/email, amount + currency, terms/notes, start/end dates, status prospect/active/completed/cancelled, timestamps); `app/Domain/Core/Repositories/SponsorshipRepository.php` per §1–§5 with the tolerant generation-read pattern; admin page modeled on `Onchain/Admin/Views/NftSpamContractsView.php` + `NftSpamContractRepository`, submenu of `bcc-system-health`, `manage_options`. If sponsor data ever renders publicly (e.g. a real AdCarousel), that is a separate slice adding a read endpoint — this schema doesn't block it. |
