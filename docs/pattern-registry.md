...
- **Cross-plugin degradation counters** — `BCC\Core\Observability\DegradationMetrics`
  ...
  **Currently wired** (Phases 1 + 1.5 + 1.6 observability initiative, 2026-05-09):
  ...
  - **NullService activations** — every fallback NullObject in `bcc-core/src/NullServices/*` records when bcc-trust isn't bound. 10 of 11 NullServices instrumented (the 11th, `NullRecalcQueueRead`, returns null which the inline health endpoint already detects as `recalcSource: 'unavailable'`):
    - `null_trust_read` / `activation` (4 read methods) + `is_suspended` (fail-closed) + `lock_active_vote_for_dispute` (fail-closed). Per-method events because security postures differ. (The `eligible_panelists` event retired with the §D5 panel and `TrustReadServiceInterface::getEligiblePanelistUserIds` — Rank Phase 6.)
  ...
  - `peepso_absence` / 19 events — every BCC writer/repo on the PeepSo boundary contributes a unique event name. Phase 1.5 (2026-05-09) expanded coverage to all 18 V-11 guards across `bcc-core/src/PeepSo/*` + `bcc-core/src/Repositories/PeepSoMessageRepository`; Rank Phase 7 (2026-08-04) added `group_writer_transfer` (`PeepSoGroupWriter::transferOwnership`, the §21.2 community-custody write), 18 → 19. Events: `status_writer_create`, `comment_writer_add`, `gif_writer_create`, `photo_writer_create`, `follow_writer_follow`, `follow_writer_unfollow`, `group_writer_join`, `group_writer_leave`, `group_writer_transfer`, `notification_writer_send`, `reaction_writer_set`, `reaction_writer_remove`, `message_writer_send_new`, `message_writer_send_in_conversation`, `message_repo_unread_count`, `message_repo_is_participant`, `message_repo_root_conversation_id`, `message_repo_participants`, `message_repo_mark_viewed`. Counter is per-call (not dedup'd) so `/system/health` shows "the comment writer silently no-opped 1247 times in the last hour" not "we logged it once."
  ...
  - `search_ft_index` / 1 event (`title_only_fallback`) — bcc-search `SearchRepository::searchCandidates` served an FT-eligible query (≥3 chars) via the title-prefix LIKE fallback because the FULLTEXT index is not installed. Content and category-name matching are silently absent for these queries. Sustained activation = the hourly `bcc_search_ensure_ft_index` self-heal isn't landing the index — rebuild from BCC System → Developer → Search Index. (Added 2026-07-19.)
  ...
  - `contribution_recovery` / 1 event (`user_eval_failed`) — bcc-trust `ContributionRecoveryEvaluator`, the daily "Trust Recovery Through Contribution" sweep over the caution/risky cohort. A per-user failure is isolated (the batch continues) and recorded here; sustained activation = the recovery sweep is silently skipping users, so the cohort stops climbing back even when they contribute. Investigate the evaluator + its read repos.
  - `attestation_synthesis` / 2 events (`event_recompute_failed` + `decay_recompute_failed`) — bcc-trust `AttestationScoreSynthesis` (Slice E), folding vouch/stand_behind attestations into the trust score. `event_recompute_failed` = a per-event recompute (on cast/revoke/reaffirm, priority-25 subscriber) threw; `decay_recompute_failed` = a per-target failure inside the daily `bcc_trust_daily_attestation_decay` sweep. Both isolated (the event/sweep continues). Sustained activation = `attestation_bonus` is going stale, so backing stops moving the score.
  - `elite_eligibility` / 4 events (`event_recompute_failed` + `dispute_recompute_failed` + `tier_celebration_failed` + `schema_unavailable`) — bcc-trust `EliteEligibilityService` (§J.12), the cross-table gate on the top reputation tier (distinct backers / account tenure / clean dispute record). `event_recompute_failed` = the priority-20 attestation subscriber threw; `dispute_recompute_failed` = the `bcc.domain.dispute_resolved` subscriber threw; `tier_celebration_failed` = the promotion notification after a 0→1 flip threw. All isolated. Sustained activation is **fail-open**: an unevaluated page stays grandfathered by `tierSql()`, i.e. still elite — so this is a trust-integrity alarm, not a cosmetic one. Backstopped by the daily `bcc_trust_elite_eligibility_sweep`.
  - `audit_log_swallow` / 4 events (2 read-path + 2 write-path) — silent-catch read paths that are supposed to be reliable, **plus** the WRITE path inside `AuditLogger::log()` itself (the §VIII.30 swallow that the constitution deliberately requires). Sustained activation on any event = the audit subsystem is unhealthy; admins see it via `/system/health` before incident review discovers the gap. Events:
    ...
    - `meta_encode_failed` (2026-09-01) — bcc-trust `AuditLogger` could not JSON-encode the caller's `$meta` after redaction. The base audit row IS still written (accountability is never traded for context), but that row's metadata is NULL. Sustained activation means some caller is passing an unserialisable payload — a metadata bug, not a DB fault, so remediation differs from `log_write_failed`.
  ...
  - **`gated_group_provision` / 3 events** (2026-05-26) — `bcc_gated_group_provision` cron sweep failure modes. Failed provisions are retried on the next tick (daily cadence); persistent activation = retry path is not catching up.
    - `peepso_absent` — PeepSoGroup class missing entirely; whole sweep short-circuits at [GatedGroupProvisioningService:42](../app/public/wp-content/plugins/bcc-trust/app/Domain/Onchain/Services/GatedGroupProvisioningService.php#L42). Distinct from the per-writer `peepso_absence` subsystem — this surface is the sweep itself, not a writer call.
    - `no_admin_owner` — no administrator user exists to own auto-provisioned groups; whole sweep short-circuits at [GatedGroupProvisioningService:48](../app/public/wp-content/plugins/bcc-trust/app/Domain/Onchain/Services/GatedGroupProvisioningService.php#L48). Rare; typically only after destructive ops on admin accounts.
    - `group_create_failed` — `new PeepSoGroup` returned a 0-id group OR its constructor threw. PER-COLLECTION event so the counter scales with the size of the failed batch; sustained activation = PeepSo Groups subsystem unhealthy even though the class loaded.

  - **`helius_dedup` / 3 events** (2026-05-26; +2 on 2026-07-20) — `replay_skipped`: Helius webhook replay-protection activations. Recorded inside [`HeliusWebhookEndpoint::handle`](../app/public/wp-content/plugins/bcc-trust/app/Domain/Onchain/REST/HeliusWebhookEndpoint.php) every time `HeliusSeenSignaturesRepository::markSeen` returns `false` (signature already seen). `ingest_failed_unmarked`: a delivery's batch ingest threw and its just-marked signatures were unmarked so Helius can redeliver (recovered, no loss). `unmark_failed`: same ingest failure but the unmark was incomplete — the batch is lost and its resend is refused as a replay (P1, manual replay needed). The dedup itself is **working** — the replay was correctly refused — but sustained nonzero activation is operationally interesting:
    - `replay_skipped` — signature was already processed; bumped alongside the existing `bcc_helius_signature_seen_total` wp_options counter (which drives the admin Helius tile). Distinct surface: the admin tile is for operators inspecting Helius volume; `/system/health` is the canonical degradation surface.
    - **Sustained activation** can be either legitimate (Helius double-sent — appears in their dashboard) or hostile (auth header stolen, attacker replaying known signatures against the always-200 endpoint). Operators discover the second case here.
  ...
  - **`cron_dispatch` / 2 events** (2026-05-26) — soft enqueue failures on the trust async surface. `wp_schedule_single_event` returning `false` (or AS returning 0) means the worker never fires. Most post-mutation paths recover via a reconciliation sweep (e.g. `DisputeScheduler::doReconcile`, the recurring graph/recalc/stats sweeps). The two events below are the surfaces where loss is **unrecoverable** without manual replay:
    ...
    - `vote_job_dispatcher` — bcc-trust `VoteJobDispatcher` enqueue failure on EITHER arm: the wp-cron fallback (`wp_schedule_single_event` → `false`) OR the Action Scheduler arm (`enqueueWithLock` → `as_enqueue_async_action` returns 0, added 2026-07-10 / bcc-trust #75). Both strand the composite `bcc_trust_async_post_vote` job, so its sub-tasks (fraud analysis, trust graph, stats refresh) silently drop for that vote.
    - **Sustained nonzero activation** = `wp_options` is unhealthy on the hot mutation path. Treat as incident, not warning.
    - **NOT instrumented** here: `DisputeNotificationService::enqueueAsync` (covered by its existing reconciliation sweep + `Logger::error` in `DisputeController`). The AS arm's *enqueue* failure IS now instrumented (#75) — a `return`-0 creates no action row, so it is invisible in the AS admin UI, unlike an action that runs and then fails.

  - **`post_commit_task` / 1 event** (2026-07-10) — a post-commit trust-processing STEP threw during **execution** and was swallowed (logged, non-fatal). Distinct from `cron_dispatch` (that's the enqueue failing / worker never firing) — here the work ran but a step errored, so this is not a `wp_options` signal:
    - `vote_subtask_failed` — a post-vote fan-out sub-task (fraud / trust-graph / stats) threw in `VoteJobDispatcher::handlePostVote`. Trust-graph + stats recover via their recurring sweeps; fraud analysis has none, so its failure is the one that leaves a stale bonus.
    - (`dispute_backfill_failed` retired — Rank Phase 6, D-7: the §D5 panelist-accuracy backfill that recorded it was deleted, and bcc-core #45 removed it from the canonical map.)
...
- **System-health snapshot extension seam** — `apply_filters('bcc_system_health', [])`
  ...
  **Currently registered contributors** (Phase 3 observability initiative, 2026-05-09):
  ...
  | bcc-core | `degradation_metrics` | `DegradationMetrics::healthSnapshot()` | `any_active` triage flag + per-subsystem current/previous-hour counts. Currently wired subsystems: `throttle`, `null_trust_read`, the 10 `null_*` NullService activations, `peepso_absence` (19 events), `search_lkg`, `read_model_fallback`, `audit_log_swallow` (4 events — 2 read-path + 2 write-path as of 2026-09-01), `legacy_ajax` (3 events — was 9; 6 wallet/collection AJAX handlers retired 2026-05-25), `account_security_mail` (7 events — AccountSecurityMailer wp_mail failures on credential / wallet change / sessions-revoked-all / password-reset-request side-channel emails; treat sustained activation as P1), `cron_dispatch` (2 events — `endorsement_fraud_analyzer` + `vote_job_dispatcher` soft enqueue failures on the unrecoverable trust async surface; sustained activation = wp_options sick, treat as incident), `post_commit_task` (1 event — `vote_subtask_failed`; swallowed post-commit trust-task *execution* failures, NOT wp_options signals; `dispute_backfill_failed` retired with the §D5 backfill, Rank Phase 6), `helius_dedup` (3 events — `replay_skipped`; Helius webhook signature already seen; sustained activation = double-send or replay attempt. `ingest_failed_unmarked`; a delivery's batch ingest threw and its signatures were unmarked so Helius can redeliver — recovered, no loss. `unmark_failed`; same ingest failure but the unmark was incomplete, so the batch is lost and its resend is refused — P1, manual replay needed), `gated_group_provision` (3 events — `peepso_absent` / `no_admin_owner` / `group_create_failed`; daily cron sweep retry-not-catching-up signal), `contribution_recovery` (1 event — `user_eval_failed`; daily trust-recovery sweep per-user failure), `rank_scoring` (4 events — `tier_snapshot_failed`; the daily tier-day snapshot sweep threw, so the §13.1 trust-window history develops gaps — fail-safe strict (a gap can only delay a promotion, never grant one) but sustained activation means promotions resolve conservatively. `login_write_failed`; a RankLoginListener login-day write threw, so that member's login day may be missing from the time-credit / inactivity clocks. `evidence_ingest_failed`; a Phase 4 rank_events ledger subscriber threw, so that evidence event is missing from the member's ledger — fail-safe strict, sustained activation means silent under-recording. `poll_close_failed`; the hourly Phase 6 meaningful-vote poll-close sweep threw, so due polls stop closing and disputes stall open in `reviewing` — fail-safe strict (no outcome is ever applied early), recovers when the sweep next succeeds). The canonical-subsystem map lives in bcc-core/bcc-core.php — new subsystems wired into `DegradationMetrics::record()` should add their `(subsystem, events)` tuple there. |
  ...
  | bcc-trust | `cron_status` | `Plugin.php:1743` | per-canonical-cron `next_run_ts` + `in_seconds`. `null` means "not currently scheduled" — alarm signal that a hook fell out of the activation registry. Covers: `bcc_disputes_reconcile_orphans`, `bcc_gated_group_provision`, `bcc_gated_group_reconcile_sweep`, `bcc_nft_eth_indexer_tick`, `bcc_nft_enrichment_tick`, `bcc_helius_dedupe_sweep`, `bcc_trust_daily_graph_update`, `bcc_trust_process_recalculations`. |
...
| Disputes | 5 | `DisputeCallout`, `DisputeDetail`, `DisputesRoom`, `MyDisputesList`, `OpenDisputeModal`. (`PanelQueue`, `PanelVoteModal` retired with the §D5 panel — Rank Phase 6.) |
