...
- **Cross-plugin degradation counters** — `BCC\Core\Observability\DegradationMetrics`
  ...
  **Currently wired** (Phases 1 + 1.5 + 1.6 observability initiative, 2026-05-09):
  ...
  - **`post_commit_task` / 2 events** (2026-07-10) — a post-commit trust-processing STEP threw during **execution** and was swallowed (logged, non-fatal). Distinct from `cron_dispatch` (that's the enqueue failing / worker never firing) — here the work ran but a step errored, so neither event is a `wp_options` signal:
    ...
    - `dispute_backfill_failed` — `DisputeResolver`'s panelist-accuracy backfill (`backfillOutcomeMatch`) threw after a dispute resolved. Non-fatal (credited rows survive; only their `outcome_match` mark is missing) and there is **no** reconciliation sweep, so sustained activation = accuracy marks silently accruing NULL.
