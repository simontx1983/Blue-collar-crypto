...
- **Cross-plugin degradation counters** — `BCC\Core\Observability\DegradationMetrics`
  ...
  **Currently wired** (Phases 1 + 1.5 + 1.6 observability initiative, 2026-05-09):
  ...
  - `audit_log_swallow` / 3 events (2 read-path + 1 write-path) — silent-catch read paths that are supposed to be reliable, **plus** the WRITE path inside `AuditLogger::log()` itself (the §VIII.30 swallow that the constitution deliberately requires). Sustained activation on any event = the audit subsystem is unhealthy; admins see it via `/system/health` before incident review discovers the gap. Events:
