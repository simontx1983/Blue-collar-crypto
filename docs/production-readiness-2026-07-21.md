...
- ***↻ Note 2026-07-22:*** an interim amendment had (correctly, at the time) recorded this gate as still-undecided because no decision was on file. Phillip's decision has since landed (Option B, above), so the gate is now **RESOLVED**. F3 content search remains **design-only / not built**, so the decision is a **forward policy**: it authorizes the shipped feed/permalink behavior and directs the future content-search build to mirror the feed's `public_all` gate — it does **not** mark content search implemented. The 2026-07-22 decision also confirms three **OPEN before-production implementation** items (documented, not built): the safe **public group-discovery / secret-group preview** (CL-FN04), the owner/moderator-controlled **`public_all` authorization** policy (CL-FN06), and **public-cache invalidation** across origin+edge on hide/delete/restore/downgrade (CL-73, which now absorbs FN-02). The **canonical group-stream visibility matrix** lives in `content-search-privacy-design.md` **Note C**. See the master tracker `docs/audit-remediation-checklist-2026-07-21.md` (CL-7C decision + CL-87 / CL-FN03–06 / CL-73).
