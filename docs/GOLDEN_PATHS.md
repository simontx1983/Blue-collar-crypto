...
- `validator_messaging` (4 events — `ambiguous_operator` (>1 verified operator claim on one validator page; surface fails closed, any occurrence is an incident), `schedule_failed` (backlog-delivery enqueue soft-failed at first activation; recurring sweep re-enqueues), `delivery_failed_terminal` (queued message exhausted MAX_ATTEMPTS; durably parked, needs `wp bcc-trust vmq` recovery), `lease_reaped` (expired processing lease returned to retryable; delivery worker died mid-row))
